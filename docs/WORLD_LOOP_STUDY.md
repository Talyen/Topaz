...
5. Press `I` (`Start` on gamepad) for the backpack. It has 16 fixed slots. The chest has 12 slots. Wood stacks to 20 per slot, with no weight limit. A harvest waits at its last chop if the full yield will not fit; it awards no Logging XP until completed.
...
- One versioned JSON save lives in Unity's persistent data directory (`topaz-save.json`). A replacement keeps a `.bak` copy. Version 1 saves migrate to the slot-based version 2 format without discarding items; an oversized legacy inventory is left untouched with saving disabled for that run. An unsupported or unreadable save also disables writes. Editor sessions, including tests, use isolated temporary directories so they cannot change the standalone player's save.
