...
This is a replaceable graybox for evaluating controls and camera motion on a Mac build. It has no enemies, damage, loot, inventory, gathering skill, or persistent world state.
...
The player faces the aim direction. The small green marker shows that direction, and the camera follows with modest aim look-ahead. During a dodge the player brightens; the code exposes an invulnerable state but there is no combat system to consume it yet. A resource pillar gains a ring when nearby and briefly turns green on interaction. It creates no item or experience.
