    ...
    public sealed class SaveRepository
    {
        ...
        public TopazSaveData Load()
        {
            ...
            if (!File.Exists(_path))
                return File.Exists(_path + ".bak")
                    ? Parse(File.ReadAllText(_path + ".bak")) : new TopazSaveData();
            try
            {
        ...
        static TopazSaveData Parse(string json)
        {
            ...
            int sourceVersion = envelope?.version ?? 0;
            TopazSaveData data = sourceVersion == 1
                ? MigrateV1(JsonUtility.FromJson<LegacySaveData>(json))
                : JsonUtility.FromJson<TopazSaveData>(json);
            if (data != null && sourceVersion == 2)
            {
                data.pickups = new List<PickupStateRecord>();
            }
            if (data != null && (sourceVersion == 2 || sourceVersion == 3))
            {
                data.regionId = TopazSaveData.HomeRegion;
            }
            if (data != null && sourceVersion >= 1 && sourceVersion <= 4)
                MigrateWorldTime(data);
            if (data == null || data.version != TopazSaveData.CurrentVersion ||
                data.day < 1 || double.IsNaN(data.worldHours) ||
                double.IsInfinity(data.worldHours) || data.worldHours < WorldClock.StartingHour ||
                data.backpackSlots == null || data.backpackSlots.Count > 16 ||
            ...
            foreach (NodeStateRecord node in data.nodes)
                if (node == null || node.nextAvailableDay != 0 ||
                    double.IsNaN(node.readyAtWorldHours) ||
                    double.IsInfinity(node.readyAtWorldHours) || node.readyAtWorldHours < 0)
                    throw new InvalidDataException("Topaz node state is invalid.");
        ...
        static void MigrateWorldTime(TopazSaveData data)
        {
            if (data.day < 1 || data.nodes == null)
                throw new InvalidDataException("Topaz legacy day state is invalid.");
            data.worldHours = WorldClock.StartingHour;
            foreach (NodeStateRecord node in data.nodes)
            {
                if (node == null || node.nextAvailableDay < 0)
                    throw new InvalidDataException("Topaz legacy node state is invalid.");
                long restsRemaining = Math.Max(0L, (long)node.nextAvailableDay - data.day);
                node.readyAtWorldHours = restsRemaining == 0 ? 0d :
                    data.worldHours + restsRemaining * WorldClock.RestHours;
                node.nextAvailableDay = 0;
            }
