...
namespace Topaz.Editor
{
    ...
    public static class KayKitSceneSetup
    {
        ...
        const string ArtRoot = "Assets/Topaz/ArtStudy";
        ...
        public static void Configure()
        {
            ...
            HarvestDefinition harvestValues = AssetDatabase.LoadAssetAtPath<HarvestDefinition>(
                "Assets/Topaz/LoopStudy/Definitions/Tree.asset");
            ...
            Renderer treeRenderer = Attach(treeVisual,
                "Forest/Models/Tree_2_A_Color1.fbx", forest, 3.5f,
                "KayKit Harvest Tree");
            ...
            AddDecoration(art.transform, "Forest/Models/Tree_1_A_Color1.fbx", forest,
                new Vector3(-8f,0f,5f), 3.4f);
            AddDecoration(art.transform, "Forest/Models/Tree_3_A_Color1.fbx", forest,
            ...
            AddDecoration(art.transform, "Forest/Models/Rock_2_A_Color1.fbx", forest,
                new Vector3(7.8f,0f,3f), 1.0f);

            Transform navigation = feel.transform.Find("Navigation Geometry");
            if (navigation != null)
            {
                string[] rocks = { "Rock_2_A_Color1.fbx", "Rock_1_A_Color1.fbx",
                    "Rock_3_A_Color1.fbx", "Rock_2_B_Color1.fbx" };
            ...
            AddSwordsHud(world.transform.Find("Loop HUD").GetComponent<LoopHud>());
        ...
        static void AddSwordsHud(LoopHud hud)
        {
            Transform resources = hud.transform.Find("Resources");
            if (resources == null) throw new InvalidOperationException("Resource HUD is missing.");
            Transform existing = resources.Find("Swords");
            TMP_Text label;
            if (existing == null)
            {
                var go = new GameObject("Swords", typeof(RectTransform), typeof(TextMeshProUGUI),
                    typeof(UnityEngine.UI.LayoutElement));
                go.transform.SetParent(resources, false);
                go.transform.SetSiblingIndex(3);
                label = go.GetComponent<TextMeshProUGUI>();
                label.font = TMP_Settings.defaultFontAsset;
                label.fontSize = 24;
                label.color = UnityEngine.Color.white;
                label.text = "SWORDS  LV 1  •  0 XP";
                label.raycastTarget = false;
                go.GetComponent<UnityEngine.UI.LayoutElement>().preferredHeight = 36;
            }
            else label = existing.GetComponent<TMP_Text>();
            label.textWrappingMode = TextWrappingModes.NoWrap;
            resources.GetComponent<RectTransform>().sizeDelta = new Vector2(410, 245);
            Ref(hud, "swordsLabel", label);
        }
