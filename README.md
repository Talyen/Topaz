...
The first command writes a machine-specific `.codex/config.toml` that is ignored by Git. `verify.sh` checks Unity Edit Mode tests and builds a Mac player. The Windows build is a cross-platform smoke check; final Windows performance must be measured on Windows hardware. Generated players, reports, Unity `Library`, and logs are ignored.
